...
class SeasonalAnalyzer:
    ...
    def predict_best_purchase_time(self, product_name, category):
        ...
        pattern = self.seasonal_patterns.get(category, {'best_month': 1, 'worst_month': 6, 'discount': 10})
        ...
        return {
            ...
            'recommendation': self.generate_recommendation(months_to_wait, pattern['discount'])
        }

    def generate_recommendation(self, months_to_wait, discount):
        if months_to_wait == 0:
            return "Идеальное время для покупки! Сезон скидок прямо сейчас."
        elif months_to_wait <= 2:
            return f"Подождите {months_to_wait} месяца, чтобы сэкономить до {discount}%"
        else:
            return f"Лучшая цена будет через {months_to_wait} месяцев. Рекомендуем подождать."